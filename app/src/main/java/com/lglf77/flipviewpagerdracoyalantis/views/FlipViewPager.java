package com.lglf77.flipviewpagerdracoyalantis.views;
...
public class FlipViewPager extends FrameLayout {
    ...
    class PageItem {
        ...
        void fill(int i) {
            pageView = Objects.requireNonNull ( pages.get ( i ) ).pageView;
            addView(pageView);
        }
    }
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (flipping) {
                    ...
                    deltaFlipDistance /= ((double) getWidth () / FLIP_DISTANCE);
    ...
    public void setAdapter(ListAdapter adapter, int activePage, int row, int maxItems) {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            assert item != null;
            item.pageView = adapter.getView(i, pages.containsKey(i) ? Objects.requireNonNull ( pages.get ( i ) ).pageView : null, this);
            pages.put(i, item);
        }
